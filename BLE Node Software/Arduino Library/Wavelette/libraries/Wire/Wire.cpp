...
void wavelette_ppi_channel_assign(uint8_t channel_num, const volatile void * event_endpoint, const volatile void * task_endpoint) 
{
  wavelette_ppi_add(channel_num, event_endpoint, task_endpoint);
}

void wavelette_ppi_channel_unassign(uint8_t channel_num)
{
	wavelette_ppi_remove(channel_num);
}
...
bool TwoWire::twi_master_init(void)
{
    ...
    */
    ...
    wavelette_ppi_channel_assign(PPI_channel, 0, 0);
...
uint8_t TwoWire::twi_master_read(uint8_t *data, uint8_t data_length, uint8_t issue_stop_condition)
{
    ...
    if ((data_length == 1) && (issue_stop_condition == 1))
    {
        ...
        wavelette_ppi_channel_assign(PPI_channel, &twi->EVENTS_BB, &twi->TASKS_STOP);
    }
    else
    {
        //rfduino_ppi_channel_assign(PPI_channel, &twi->EVENTS_BB, &twi->TASKS_SUSPEND);
        wavelette_ppi_channel_assign(PPI_channel, &twi->EVENTS_BB, &twi->TASKS_SUSPEND);
    }
    twi->TASKS_STARTRX = 1;
    while(true)
    {
